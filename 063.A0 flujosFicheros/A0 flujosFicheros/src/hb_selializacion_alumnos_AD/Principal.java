...
public class Principal {
	public static void graba(boolean append) throws IOException {
		...
		ArrayList<Alumno> lAlu = new ArrayList<Alumno>(Arrays.asList(alumnos));
		try {
			ad.getSalida().writeObject(lAlu);
		} catch (IOException e) {
			e.printStackTrace();
		}
		ad.cierra();
	...
	public static void lee(String nombre) throws IOException, ClassNotFoundException {
		AccesoDatos ad = new AccesoDatos(nombre);
		ad.abreEntrada();
		ArrayList<Alumno> lAlu = (ArrayList<Alumno>) ad.getEntrada().readObject();
		for (Alumno a : lAlu)
			System.out.println(a);
		ad.cierra();
	}

	public static void parte() throws ClassNotFoundException {
