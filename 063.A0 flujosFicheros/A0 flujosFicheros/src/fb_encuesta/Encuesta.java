...
public class Encuesta implements Constantes {
	...
	public void creaExitos() throws IOException {
		Exito exito;
		Random r= new Random();
		ExitoAccesoDatos gestion = new ExitoAccesoDatos(true);
		for (int e = 0; e < NEXITOS ; e++){
			int n= e*2 + 1; // * 2 + 1;
			exito = new Exito(n, "Titulo Album " + n);
			gestion.grabaRegistro(n, exito);
		}
		gestion.cierra();
	}
	/**
	...
	public void listadoSimpleExitos01() throws IOException {
		...
		gestion = new ExitoAccesoDatos(false);
	...
	public void procesaEncuestaDirecto() throws IOException {
		ExitoAccesoDatos gestionE = null;
		PersonaAccesoDatos gestionP = null;
		Persona p = null;
		Exito exito = null;
		...
		try {
			...
			while (!fF) {
				for (int ve : p.getVotos())
					try { // o si retorna null
						exito = gestionE.leeRegistro(ve);
						exito.incrementaVotos();
						gestionE.grabaRegistro(exito.getNumero(), exito);
					} catch (NullPointerException e) {
					}
				p = gestionP.leeRegistro();
			}
	...
	public static void main(String[] s) {
		...
		encuesta.listadoSimpleExitos01();
		//encuesta.procesaEncuestaDirecto();
